...
# 定义文件路径
file_name = "D:\\dizhenshujuji\\stead_data\\chunk1.hdf5 "  # HDF5 数据文件
csv_file = "D:\\dizhenshujuji\\stead_data\\chunk1.csv"  # CSV 文件
...
counter = 0  # 添加计数器
noise_data_list = []  # 创建一个列表用于保存噪声数据
...
# 保存噪声数据列表到单独的 .npy 文件中
for i, data in enumerate(noise_data_list):
    np.save(f'noise_data_{i}.npy', data)